...
with tabs[0]:
    ...
    # Safe retrieval for risk_score / failure_probability
    risk_score_raw = agent_data["predictive"].get("risk_score", agent_data["predictive"].get("failure_probability", 0.0))
    risk_score_pct = risk_score_raw * 100 if risk_score_raw <= 1.0 else risk_score_raw
...
with tabs[3]:
    ...
    with vcol2:
        ...
        if os.path.exists("data/factory_sop.txt"):
            # SAFE UTF-8 READING FIX INCLUDED HERE
            with open("data/factory_sop.txt", "r", encoding="utf-8", errors="ignore") as f:
                st.text(f.read())
...
with tabs[4]:
    ...
    # Safe dictionary parsing for agent steps
    planning_info = agent_data.get("planning", agent_data.get("consensus", {}))
    urgency_val = planning_info.get("urgency", planning_info.get("priority", "N/A"))
    recommendations_val = planning_info.get("recommendations", planning_info.get("recommended_action", "N/A"))

    with ca1:
        st.markdown(f'<div class="agent-card"><b>👁️ Vision Agent</b><br><small>{agent_data["vision"].get("details", "N/A")}</small></div>', unsafe_allow_html=True)
    with ca2:
    ...
    with ca3:
        st.markdown(f'<div class="agent-card"><b>📚 Knowledge Agent</b><br><small>{str(agent_data["knowledge"])[:45]}...</small></div>', unsafe_allow_html=True)
    with ca4:
        st.markdown(f'<div class="agent-card"><b>🧠 Planning Agent</b><br><small>Urgency: <b>{urgency_val}</b></small></div>', unsafe_allow_html=True)
